...
char original_Password[15]="NIET";
do
{
...
D:
   ...
   if(strlen(p.PhoneNumber)>10||strlen(p.PhoneNumber)!=10)
{
printf("\n\t Sorry Invalid Numbers. Please Enter 10 digit numbers without country code.");
goto D;
}
else
{
...
do
{
   ...
   if (strlen(p.Email)>30||strlen(p.Email)<8)
   {
     ...
     for(i = 0;p.Email[i]!='\0'; i++) 
		 {
             if (p.Email[i] != '@')
  			 printf("\n\t Invalid.Must Include @ in an Email Address. ");
} }
